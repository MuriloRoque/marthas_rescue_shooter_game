...
import Multirole from '../models/enemy/multirole';
import Boss6 from '../models/bosses/chapter6_boss';

let life1;
let life2;
let life3;

export default class GameScene6 extends Phaser.Scene {
  constructor () {
    super('Game6');
  }
  ...
  create () {
    ...
    if (this.music.musicOn === true) {
      ...
      this.bgMusic = this.sound.add('warzoneMusic', { volume: 0.5, loop: true });
      this.bgMusic.play();
    ...
    this.time.addEvent({
      ...
      callback: function() {
        this.scene.pause();
        this.scene.launch("Chapter6BossDialogue")
        this.stopEnemy = true;
        var boss = null;

        var boss = new Boss6(this, 400, 80, 25);
    ...
    this.physics.add.collider(this.playerMissiles, this.enemies, function(playerMissile, enemy) {
      if (enemy) {
        if(enemy.constructor.name === "Boss6"){
          enemy.hp -= 1;
          ...
          if(enemy.hp === 0){
            ...
            myself.scene.start("Chapter6EndDialogue1");
          }
        }
        else{
