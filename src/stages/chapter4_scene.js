...
export default class GameScene4 extends Phaser.Scene {
  ...
  create () {
    ...
    if (this.music.musicOn === true) {
      ...
      this.bgMusic = this.sound.add('forestMusic', { volume: 0.5, loop: true });
      this.bgMusic.play();
    ...
    this.time.addEvent({
      ...
      callback: function() {
        this.scene.pause();
        this.scene.launch("Chapter4BossDialogue")
        this.stopEnemy = true;
        var boss = null;

        var boss = new Boss4(this, 400, 80, 25);
    ...
    this.physics.add.collider(this.playerMissiles, this.enemies, function(playerMissile, enemy) {
      if (enemy) {
        if(enemy.constructor.name === "Boss4"){
          enemy.hp -= 1;
          ...
          if(enemy.hp === 0){
            ...
            myself.scene.start("Chapter4EndDialogue");
          }
        }
        else{
