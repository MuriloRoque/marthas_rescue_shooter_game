...
export default class GameScene3 extends Phaser.Scene {
  ...
  create () {
    ...
    this.time.addEvent({
      ...
      callback: function() {
        this.scene.pause();
        this.scene.launch("Chapter3BossDialogue")
        this.stopEnemy = true;
        var boss = null;

        var boss = new Boss3(this, 400, 80, 25);
    ...
    this.physics.add.collider(this.playerMissiles, this.enemies, function(playerMissile, enemy) {
      if (enemy) {
        if(enemy.constructor.name === "Boss3"){
          enemy.hp -= 1;
          ...
          if(enemy.hp === 0){
            ...
            myself.scene.start("Chapter3EndDialogue");
          }
        }
        else{
